...
async function routeCallback(ctx: Context, action: string, args: string[]): Promise<void> {
  ...
  switch (action) {
    ...
    case 'copy_decisions':
      return T.showCopyDecisions(ctx);
    ...
    case 'safety_rugger':
      return T.cycleSafety(ctx, 'rugger');
    case 'safety_insider':
      return T.cycleSafety(ctx, 'insider');
    case 'safety_factory':
      return T.cycleSafety(ctx, 'factory');
    case 'safety_traders':
      return T.cycleSafety(ctx, 'traders');
